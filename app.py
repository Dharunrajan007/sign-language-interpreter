...
def main():
    ...
    # Warmup camera (read and discard first few frames to avoid black output)
    for _ in range(5):
        cap.read()

    # Model load - using new mediapipe Tasks API
    base_options = mp_tasks.BaseOptions(model_asset_path=MODEL_PATH)
    options = mp_vision.HandLandmarkerOptions(
        base_options=base_options,
        running_mode=mp_vision.RunningMode.VIDEO if not use_static_image_mode else mp_vision.RunningMode.IMAGE,
        num_hands=1,
        ...
        if detection_result.hand_landmarks:
            for hand_idx, hand_landmarks in enumerate(detection_result.hand_landmarks):
                ...
                if detection_result.handedness and hand_idx < len(detection_result.handedness):
                    handedness_label = detection_result.handedness[hand_idx][0].display_name
                else:
                ...
                if hand_sign_text == "EMERGENCY":
                    cv.putText(debug_image, "!!! ALERT !!!", (50, 100),
                               cv.FONT_HERSHEY_SIMPLEX, 2.0, (0, 0, 255), 4, cv.LINE_AA)
